...
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-9">
                <div class="row header-image" style="background-image: url('{{ url('storage/' . $restaurant->image) }}')">
                </div>
                <div class="row information">
                    ...
                    <div id="favorite-button" class="ml-auto my-auto" data-restaurant="{{ $restaurant->slug }}"
                        data-fav="{{ $favorite ? 'true' : 'false'}}"></div>
                    <div class="info-icon">
                        <a class="nav-link" title="More info" data-toggle="modal" aria-labelledby="modal-default"
                            data-target="#modal-default" aria-hidden="true" role="dialog" aria-selected="false"><i
                            ...
                            <div class="category" id="{{$category->name}}">
                                ...
                                @foreach($menus as $menu)
                                    @if($menu->category_id == $category->id)
                                        <a class="menu-link" data-toggle="modal"
                                            data-target="{{ $menu->available ? '#modal-menu-' . $menu->id : '' }}">
                                        ...
                                        @if($menu->available)
                                            <div class="modal fade" id="modal-menu-{{$menu->id}}" tabindex="-1" role="dialog"
                                                aria-labelledby="modal-menu-{{$menu->id}}" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <form method="POST" action="{{ route('cart.store', $menu->id) }}">
                                                            @csrf
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="modal-title-menu-{{$menu->id}}">{{$menu->name}}
                                                                </h4>
                                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                    <span aria-hidden="true">×</span>
                                                                </button>
                                                            ...
                                                            <div class="modal-body text-center">
                                                                ...
                                                                <div class="center-block">
                                                                    <input name="quantity" type="number" value="1" min="1" max="20" step="1"
                                                                        required />
                                                                </div>
                                                            </div>
